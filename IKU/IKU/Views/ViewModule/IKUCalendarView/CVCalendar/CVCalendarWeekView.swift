...
public final class CVCalendarWeekView: UIView {
    ...
    public init(monthView: CVCalendarMonthView, index: Int) {
        ...
        if let size = monthView.calendarView.weekViewSize {
            let hSpace = monthView.calendarView.appearance.spaceBetweenWeekViews ?? 2.0
            let y = CGFloat(index) * (size.height + hSpace)
            super.init(frame: CGRect(x: 0, y: y, width: size.width, height: size.height))
        } else {
