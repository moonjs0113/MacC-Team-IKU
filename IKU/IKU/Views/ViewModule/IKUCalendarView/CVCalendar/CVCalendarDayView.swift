...
public final class CVCalendarDayView: UIView {
    ...
    public override func layoutSubviews() {
        ...
        if let toDayCircleView = self.toDayCircleView {
            toDayCircleView.layer.cornerRadius = self.frame.width / 4
        }
    }
}
...
//        dayLabel?.frame = bounds
        ...
        let font: UIFont? = .nexonGothicFont(ofSize: 13, weight: isCurrentDay ? .bold : .regular)
//        appearance?.dayLabelWeekdayFont
//        var color: UIColor?
...
//        }
        ...
        dayLabel.textColor = (weekdayIndex == 1 || weekdayIndex == 7) ? .ikuCalendarWeekend : .black
        ...
        toDayCircleView = UIView()
        toDayCircleView.backgroundColor = isCurrentDay ? .ikuBlue : .white
        addSubview(toDayCircleView)
        toDayCircleView.translatesAutoresizingMaskIntoConstraints = false
        toDayCircleView.clipsToBounds = true
        NSLayoutConstraint.activate([
            toDayCircleView.topAnchor.constraint(equalTo: topAnchor, constant: 2),
            toDayCircleView.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 2),
            toDayCircleView.widthAnchor.constraint(equalTo: widthAnchor, multiplier: 1/2),
            toDayCircleView.heightAnchor.constraint(equalTo: toDayCircleView.widthAnchor),
        ])
        
        toDayCircleView.addSubview(dayLabel)
        dayLabel.translatesAutoresizingMaskIntoConstraints = false
        
        NSLayoutConstraint.activate([
            dayLabel.centerXAnchor.constraint(equalTo: toDayCircleView.centerXAnchor),
            dayLabel.centerYAnchor.constraint(equalTo: toDayCircleView.centerYAnchor),
            dayLabel.heightAnchor.constraint(equalTo: toDayCircleView.heightAnchor),
            dayLabel.widthAnchor.constraint(equalTo: toDayCircleView.widthAnchor),
        ])
    }
    
    public func iconSetup() {
        dayIcon = UIImageView(image: UIImage(systemName: "magnifyingglass.circle"))
        dayIcon.tintColor = .ikuBlue
        dayIcon.contentMode = .scaleAspectFill
        
        addSubview(dayIcon)
        dayIcon.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([
            dayIcon.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -7),
