      ...
      safeExecuteBlock({
            for (index, weekView) in self.weekViews.enumerated() {
                if let size = self.calendarView.weekViewSize {
                    let vSpace = self.calendarView.appearance.spaceBetweenWeekViews ?? 2.5
                    let y = CGFloat(index) * (size.height + vSpace)
                    weekView.frame = CGRect(x: 0, y: y, width: size.width, height: size.height)
